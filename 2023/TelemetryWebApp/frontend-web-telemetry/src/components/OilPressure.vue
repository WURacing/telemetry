...
const OilPressure = ref([] as number[]);
...
watch([() => store.Time, () => store.OilPressure], ([newTime, newOilPressure]) => {
  // Update the local GPSXPos and GPSYPos variables with the new values
  Time.value = newTime;
  OilPressure.value = newOilPressure;
...
async function initSciChart() {
  ...
  const { sciChartSurface, wasmContext } = await SciChartSurface.create("scichart-oilPressure", {
  ...
  sciChartSurface.yAxes.add(new NumericAxis(wasmContext, { axisTitle: "psi", autoRange: EAutoRange.Always}));
  ...
  for (let i = 0; i < Time.value.length; i++) {
    ...
    yValues.push(OilPressure.value[i]);
  }
